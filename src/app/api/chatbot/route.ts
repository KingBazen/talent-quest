...
async function callAnthropic(query: string, lang: "en" | "am"): Promise<string> {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system:
        lang === "am"
          ? "You are Stage Bot for TalentQuest Ethiopia. Answer in Amharic. Be concise (under 80 words). Topics: registration, video submission, judging, payment (Telebirr), schedule. If unsure, say so."
          : "You are Stage Bot for TalentQuest Ethiopia. Answer in English. Be concise (under 80 words). Topics: registration, video submission, judging, payment (Telebirr), schedule. If unsure, say so.",
